...
searchForm.addEventListener("submit", (e) => {
  ...
  if (query) {
    window.location.href = `https://chatgpt.com/?model=gpt-4&q=${encodeURIComponent(
      query
    )}&hints=search`;
  } else {
...
document.addEventListener("mousemove", (e) => {
  ...
  document.querySelector(
    ".content"
  ).style.transform = `translate(-50%, -50%) rotateY(${xAxis}deg) rotateX(${yAxis}deg) translateZ(20px)`;
});
    ...
    for (let i = 0; i < particles.length; i++) {
      ...
      if (mouse.x && mouse.y) {
        ...
        if (distance < mouse.radius * window.devicePixelRatio) {
          const force =
            ((mouse.radius * window.devicePixelRatio - distance) /
              (mouse.radius * window.devicePixelRatio)) *
            0.6;
